...
object LeadRequestRepository {
    ...
    val leadRequestSetterGetter = MutableLiveData<LeadRequestModel>()
    val TAG: String = "LeadRequestRepository"

    fun getServicesApiCall(context: Context): MutableLiveData<LeadRequestModel> {
        getLeadRequest(context)
        return leadRequestSetterGetter
    }

    private fun getLeadRequest(context: Context) {

        try {
            leadRequestSetterGetter.value =  LeadRequestModel("")
...
//                    "Token":"UJKVOZNetDYdsvmY84dsd5q5e7F+2nmEciW+y5WDWAOyVD2b4auSWw==\n","Name":"j4rFcTOFBx0=\n","Todate":"j4rFcTOFBx0=\n","criteria":"j4rFcTOFBx0=\n"}
            ...
            val call = apiService.getWalkingCustomerDetailsList(body)
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<LeadRequestModel>()
                        leads.add(LeadRequestModel(response.body()))
                        val msg = leads[0].message
                        leadRequestSetterGetter.value = LeadRequestModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
