...
object AccBankBalanceRepository {
    private var progressDialog: ProgressDialog? = null
    val accBankBalanceSetGet = MutableLiveData<AccBankBalanceModel>()
    val TAG: String = "AccBankBalanceRepository"

    fun getServicesApiCall(context: Context, TransDate : String, DashMode : String, DashType : String): MutableLiveData<AccBankBalanceModel> {
        getAccBankBalance(context, TransDate, DashMode, DashType)
        return accBankBalanceSetGet
    }

    private fun getAccBankBalance(context: Context, TransDate: String, DashMode: String, DashType: String) {
        try {
            accBankBalanceSetGet.value = AccBankBalanceModel("")
            ...
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<AccBankBalanceModel>()
                        leads.add(AccBankBalanceModel(response.body()))
                        val msg = leads[0].message
                        accBankBalanceSetGet.value = AccBankBalanceModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
                        Log.e(TAG,"1151  "+e)
