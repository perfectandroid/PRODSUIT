...
object ProductreorderLevelRepository {

    private var progressDialog: ProgressDialog? = null
    val productreorderLevelSetterGetter = MutableLiveData<ProductReorderLevelModel>()
    val TAG: String = "ProductreorderLevelRepository"

    fun getServicesApiCall(context: Context): MutableLiveData<ProductReorderLevelModel> {
        getProductPriority(context)
        return productreorderLevelSetterGetter
    }
    private fun getProductPriority(context: Context) {
        try {
            productreorderLevelSetterGetter.value = ProductReorderLevelModel("")
            ...
            try {
                ...
                requestObject1.put("DashMode", ProdsuitApplication.encryptStart("27"))
            ...
            val call = apiService.getInventoryProductReorderLevel(body)
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<ProductReorderLevelModel>()
                        leads.add(ProductReorderLevelModel(response.body()))
                        val msg = leads[0].message
                        productreorderLevelSetterGetter.value = ProductReorderLevelModel(msg)
                    } catch (e: Exception) {
                        e.printStackTrace()
                        progressDialog!!.dismiss()
