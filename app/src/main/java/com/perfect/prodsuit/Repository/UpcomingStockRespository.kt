...
object UpcomingStockRespository {

    private var progressDialog: ProgressDialog? = null
    val upcomingStockRespositorySetterGetter = MutableLiveData<UpcomingStockModel>()
    val TAG: String = "UpcomingStockRespository"

    fun getServicesApiCall(context: Context,TransDate: String): MutableLiveData<UpcomingStockModel> {
        getUpcomingStock(context,TransDate)
        return upcomingStockRespositorySetterGetter
    }

    private fun getUpcomingStock(context: Context,TransDate: String) {
        try {
            upcomingStockRespositorySetterGetter.value = UpcomingStockModel("")
...
//                requestObject1.put("TransDate", ProdsuitApplication.encryptStart("2023-11-08"))
            ...
            val call = apiService.getProductionUpcomingStock(body)
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<UpcomingStockModel>()
                        leads.add(UpcomingStockModel(response.body()))
                        val msg = leads[0].message
                        upcomingStockRespositorySetterGetter.value = UpcomingStockModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
