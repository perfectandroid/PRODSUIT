...
object ProjectPayInfoRepository {

    private var progressDialog: ProgressDialog? = null
    val projectPayInfoSetGet = MutableLiveData<ProjectPayInfoModel>()
    val TAG: String = "ProjectPayInfoRepository"

    fun getServicesApiCall(context: Context, ID_Project : String, ID_Stage : String, ID_PettyCashier : String, ID_Employee : String): MutableLiveData<ProjectPayInfoModel> {
        getprojectPayInfo(context,ID_Project,ID_Stage,ID_PettyCashier,ID_Employee)
        return projectPayInfoSetGet
    }

    private fun getprojectPayInfo(context: Context, ID_Project : String, ID_Stage : String, ID_PettyCashier : String, ID_Employee : String) {
        try {
            projectPayInfoSetGet.value = ProjectPayInfoModel("")
            ...
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<ProjectPayInfoModel>()
                        leads.add(ProjectPayInfoModel(response.body()))
                        val msg = leads[0].message
                        projectPayInfoSetGet.value = ProjectPayInfoModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
                        Log.e(TAG,"1151  "+e)
