...
object leadstagecountwiseRepository {

    private var progressDialog: ProgressDialog? = null
    val stockListCategorySetterGetter = MutableLiveData<leadstagecountwiseModel>()
    val TAG: String = "StockListCategoryRepository"

    fun getServicesApiCall(context: Context): MutableLiveData<leadstagecountwiseModel> {
    ...
    private fun getStockListCategory(context: Context) {
        try {
            stockListCategorySetterGetter.value = leadstagecountwiseModel("")
...
//                requestObject1.put("TransDate", ProdsuitApplication.encryptStart(currentDate))
                requestObject1.put("TransDate", ProdsuitApplication.encryptStart(currentDate))
                requestObject1.put("DashMode", ProdsuitApplication.encryptStart("4"))
                requestObject1.put("DashType", ProdsuitApplication.encryptStart("2"))
                requestObject1.put("ID_TokenUser", ProdsuitApplication.encryptStart(ID_TokenUserSP.getString("ID_TokenUser", null)))



                Log.e(TAG,"78575  leadStageCountWiseForecast  "+requestObject1)
            } catch (e: Exception) {
                e.printStackTrace()
            }
            ...
            val call = apiService.getLeadStageCountWiseForecast(body)
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        stockListCategorySetterGetter.value = leadstagecountwiseModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
