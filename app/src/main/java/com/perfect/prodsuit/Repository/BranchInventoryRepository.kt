...
object BranchInventoryRepository {
    ...
    val branchInventorySetterGetter = MutableLiveData<BranchInventoryModel>()
    val TAG: String = "BranchInventoryRepository"

    fun getServicesApiCall(context: Context, ID_BranchType : String): MutableLiveData<BranchInventoryModel> {
        getBranchInventory(context, ID_BranchType)
        return branchInventorySetterGetter
    }

    private fun getBranchInventory(context: Context, ID_BranchType: String) {
        try {
            branchInventorySetterGetter.value = BranchInventoryModel("")
            ...
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<BranchInventoryModel>()
                        leads.add(BranchInventoryModel(response.body()))
                        val msg = leads[0].message
                        branchInventorySetterGetter.value = BranchInventoryModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
