...
object CRMcomplaintwiseRepository {

    private var progressDialog: ProgressDialog? = null
    val crmcomplaintwiseSetGet = MutableLiveData<CRMcomplaintwiseModel>()
    val TAG: String = "CRMcomplaintwiseRepository"

    fun getServicesApiCall(context: Context, TransDate : String, DashMode : String, DashType : String): MutableLiveData<CRMcomplaintwiseModel> {
        getCRMcomplaintwise(context, TransDate, DashMode, DashType)
        return crmcomplaintwiseSetGet
    }

    private fun getCRMcomplaintwise(context: Context, TransDate: String, DashMode: String, DashType: String) {
        try {
            crmcomplaintwiseSetGet.value = CRMcomplaintwiseModel("")
            ...
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<CRMcomplaintwiseModel>()
                        leads.add(CRMcomplaintwiseModel(response.body()))
                        val msg = leads[0].message
                        crmcomplaintwiseSetGet.value = CRMcomplaintwiseModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
                        Log.e(TAG,"1151  "+e)
