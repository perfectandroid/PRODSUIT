...
object CallStatusRepository {
    ...
    val callStatusSetterGetter = MutableLiveData<CallStatusModel>()
    val TAG: String = "CallStatusRepository"

    fun getServicesApiCall(context: Context): MutableLiveData<CallStatusModel> {
        getLeadBy(context)
        return callStatusSetterGetter
    }

    private fun getLeadBy(context: Context) {

        try {
            callStatusSetterGetter.value = CallStatusModel("")
            ...
            try {
                ...
                requestObject1.put("IsAdmin", ProdsuitApplication.encryptStart(isAdminSP.getString("IsAdmin", null)))
                requestObject1.put("ID_TokenUser", ProdsuitApplication.encryptStart(ID_TokenUserSP.getString("ID_TokenUser", null)))



                Log.e(TAG,"78  getCallStatus  "+requestObject1)
            } catch (e: Exception) {
                e.printStackTrace()
            }
            ...
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        Log.e("CallStatus  ",response.body())
                        val leads = ArrayList<BranchModel>()
                        leads.add(BranchModel(response.body()))
                        val msg = leads[0].message
                        callStatusSetterGetter.value = CallStatusModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
