...
object AccSupplierChartRepository {

    private var progressDialog: ProgressDialog? = null
    val accSupplierChartSetGet = MutableLiveData<AccSupplierChartModel>()
    val TAG: String = "AccSupplierChartRepository"

    fun getServicesApiCall(context: Context, TransDate : String, DashMode : String, DashType : String): MutableLiveData<AccSupplierChartModel> {
        getAccSupplierChart(context, TransDate, DashMode, DashType)
        return accSupplierChartSetGet
    }

    private fun getAccSupplierChart(context: Context, TransDate: String, DashMode: String, DashType: String) {
        try {
            accSupplierChartSetGet.value = AccSupplierChartModel("")
...
//                    "FK_Company":"1","FK_BranchCodeUser":"3","TransDate":"2023-11-08","FK_Module":"5"}
            ...
            val call = apiService.getDashAccSupplierOutstandingChart(body)
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<AccSupplierChartModel>()
                        leads.add(AccSupplierChartModel(response.body()))
                        val msg = leads[0].message
                        accSupplierChartSetGet.value = AccSupplierChartModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
                        Log.e(TAG,"1151  "+e)
