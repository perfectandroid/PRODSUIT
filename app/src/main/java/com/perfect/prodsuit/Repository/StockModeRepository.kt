...
object StockModeRepository {

    private var progressDialog: ProgressDialog? = null
    val stockModeSetterGetter = MutableLiveData<StockModeModel>()
    val TAG: String = "StockModeRepository"

    fun getServicesApiCall(context: Context): MutableLiveData<StockModeModel> {
        getStockMode(context)
        return stockModeSetterGetter
    }

    private fun getStockMode(context: Context) {

        try {
            stockModeSetterGetter.value = StockModeModel("")
            ...
            call.enqueue(object : retrofit2.Callback<String> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        val leads = ArrayList<StockModeModel>()
                        leads.add(StockModeModel(response.body()))
                        val msg = leads[0].message
                        stockModeSetterGetter.value = StockModeModel(msg)
                    } catch (e: Exception) {
                        progressDialog!!.dismiss()
