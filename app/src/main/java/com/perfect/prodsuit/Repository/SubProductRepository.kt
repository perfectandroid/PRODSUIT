...
object SubProductRepository {

    private var progressDialog: ProgressDialog? = null
    val subProductSetterGetter = MutableLiveData<SubProductModel>()
    val TAG: String = "SubProductRepository"

    fun getServicesApiCall(context: Context, FkEmployee : String,

                           productDetails : JSONArray): MutableLiveData<SubProductModel> {
    ...
    private fun getSubProduct(context: Context,FkEmployee : String,productDetails : JSONArray)
    {
       // Log.i("resp900","array==   "+productDetails)
        try {
            subProductSetterGetter.value = SubProductModel("")
...
//                    "EntrBy":"APP","FK_Branch":"3",Assignees{["FK_Employee":"1","EmployeeType":"3"]}
            ...
            val call = apiService.getSubProductDetails(body)
                ...
                override fun onResponse(
                    ...
                    try {
                     ...
                     //   Log.i("resp900","response==   "+response.body())
                        ...
                        val leads = ArrayList<SubProductModel>()
                        leads.add(SubProductModel(response.body()))
                        val msg = leads[0].message
                        subProductSetterGetter.value = SubProductModel(msg)
                    } catch (e: Exception) {
                        e.printStackTrace()
                        progressDialog!!.dismiss()
